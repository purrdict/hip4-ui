      ...
      {markets.map((market) => {
        ...
        return (
          <MarketCard
            ...
            market={market}
            yesMid={yesMid ?? 0.5}
            onClick={() => navigate(`/market/${market.yesOutcome.coin}`)}
          />
        );
      })}
    </div>
...
| `orderbook` | `npx shadcn@latest add https://ui.purrdict.xyz/r/orderbook.json` | `useOrderbook` | `bids`, `asks`, `spread?`, `midPrice?`, `onPriceClick?`, `userOrders?` |
...
Orderbook         -> useOrderbook     (bids, asks, spread, midPrice)
